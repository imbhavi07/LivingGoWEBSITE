  ...
  return (
    <>
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 md:py-10 lg:px-8">
        <Gallery images={displayedImages} title={property?.title || "Property"}/>
...
<aside className="h-fit rounded-3xl bg-white p-6 shadow-soft">
  ...
  <div className="mt-3 flex grid grid-cols-2 gap-2">
    ...
    <a
      href={`https://wa.me/919355405403?text=${encodeURIComponent(
        `Hi, I'd like to schedule a visit for property *${property?.propertyCode ?? ""}* on LivingGo. Please let me know the available slots.`
      )}`}
      target="_blank"
      rel="noopener noreferrer"
      className="flex items-center justify-center gap-2 rounded-2xl border-2 border-[#25D366] bg-white px-4 py-3 text-sm font-bold text-[#25D366] hover:bg-green-50 transition-colors"
    >
