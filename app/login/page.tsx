...
function LoginForm() {
  ...
  const activeAuth = mode === "student" ? studentAuth : ownerAuth;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      ...
      studentAuth.setError(null);
      ownerAuth.setError(null);
      return;
    }

    setError(null);
    await activeAuth.signIn(parsed.data);
  }
  ...
  return (
    <main className="mx-auto grid min-h-[76vh] max-w-6xl items-center gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
      <section className="relative overflow-hidden rounded-[2rem] bg-ink p-6 text-white shadow-lift sm:p-8 lg:min-h-[560px]">
        <div
          className={cn(
            "absolute inset-x-8 top-10 h-48 rounded-full blur-3xl transition-all duration-700",
            mode === "student" ? "bg-clay/35" : "translate-y-40 bg-moss/40"
          )}
        />
        <div
          className={cn(
            "absolute -right-20 top-20 h-56 w-56 rounded-full border border-white/10 transition duration-700",
            mode === "owner" && "translate-x-[-60px] translate-y-28 scale-125"
          )}
        />
        <div className="relative z-10 flex min-h-[440px] flex-col justify-between">
          ...
          <div className="py-10">
            <div className="relative mb-8 h-28">
              <div
                className={cn(
                  "absolute left-0 top-0 rounded-[2rem] bg-white/10 p-6 transition-all duration-700",
                  mode === "student" ? "rotate-0 scale-100 opacity-100" : "-translate-y-4 rotate-[-8deg] scale-75 opacity-30"
                )}
              >
              ...
              <div
                className={cn(
                  "absolute left-20 top-8 rounded-[2rem] bg-white/10 p-6 transition-all duration-700",
                  mode === "owner" ? "translate-x-0 rotate-0 scale-100 opacity-100" : "translate-x-10 rotate-6 scale-75 opacity-30"
                )}
              >
      ...
      <section className="rounded-[2rem] bg-white p-5 shadow-soft sm:p-8">
        <div className="relative grid rounded-full bg-linen p-1 sm:grid-cols-2">
          <span
            className={cn(
              "absolute bottom-1 top-1 w-[calc(50%-0.25rem)] rounded-full bg-ink shadow-soft transition-transform duration-500 ease-out",
              mode === "owner" && "translate-x-full"
            )}
          />
          <button
            type="button"
            ...
            className={cn(
              "relative z-10 flex min-h-12 items-center justify-center gap-2 rounded-full text-sm font-black transition",
              mode === "student" ? "text-white" : "text-muted"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "relative z-10 flex min-h-12 items-center justify-center gap-2 rounded-full text-sm font-black transition",
              mode === "owner" ? "text-white" : "text-muted"
            )}
          >
        ...
        <form onSubmit={handleSubmit} className="mt-8">
          <div className="overflow-hidden">
            <div
              key={mode}
              className="animate-[loginPanel_420ms_ease-out] rounded-3xl bg-linen p-5 sm:p-6"
            >
              <p className="text-sm font-bold uppercase text-clay">{mode === "student" ? "Welcome student" : "Welcome owner"}</p>
              <h2 className="mt-2 text-3xl font-black text-ink">
                {mode === "student" ? "Sign in to explore homes" : "Sign in to manage listings"}
              </h2>
          ...
          {error || activeAuth.error ? (
            <p className="mt-4 rounded-2xl bg-linen p-3 text-sm font-semibold text-clay">{error ?? activeAuth.error}</p>
          ) : null}

          <Button className="mt-5 w-full" disabled={activeAuth.isSubmitting}>
            {activeAuth.isSubmitting ? "Signing in..." : mode === "student" ? "Continue as student" : "Continue as owner"}
          ...
          <div className="mt-5 text-center text-sm text-muted">
            {mode === "owner" ? (
              <p>
                New owner?{" "}
                <Link href="/owner/signup" className="font-bold text-ink">
