...
import logo from "@/assets/logo.png"; // Adjust the relative path (e.g., "../../assets/logo.png") if needed
        ...
        <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-8">
          <Link href="/" className="text-2xl font-black tracking-tight text-ink">
            <Image 
              src={logo} 
              alt="LivingGo Logo" 
              width={992} // Set to your logo's actual width
              height={597} // Set to your logo's actual height
              className="h-20 w-auto" // Tailwind to control the display size dynamically
            />
          </Link>
          ...
          {user ? (
            <div className="relative" ref={dropdownRef}>
              ...
              {/* 1. Changed parent <button> to a <div> to prevent invalid HTML nesting */}
              <div className="flex items-center gap-2 rounded-full bg-white px-4 py-2 shadow-soft transition hover:bg-linen">
      
              {/* 2. THE ICON: Wrapped in a Link for conditional redirection */}
              <Link 
                href={
              ...
              {/* 3. THE DROPDOWN TRIGGER: Name, role, and chevron */}
              <button 
