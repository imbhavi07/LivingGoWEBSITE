...
function hasCityMetro(location: string): boolean {
  const lower = location.toLowerCase();
  return METRO_CITIES.some((city) => lower.includes(city));
}

async function queryOverpass(query: string): Promise<{ elements: { tags?: Record<string, string>; lat?: number; lon?: number; center?: { lat: number; lon: number } }[] }> {
...
export async function findNearbyPlaces(
  ...
  const allDestinations: [number, number][] = [
    ...selectedColleges.map((c): [number, number] => [c.lat, c.lng]),
    ...metros.slice(0, 1).map((m): [number, number] => [m.lat, m.lng]),
  ];
  ...
  // Build result
  const collegeResults: NearbyPlace[] = selectedColleges.map((college, i) => ({
    name: college.name,
  ...
  let metroResult: NearbyPlace | null = null;
  if (metros.length > 0) {
