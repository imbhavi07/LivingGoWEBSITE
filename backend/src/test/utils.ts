import { Request, Response, NextFunction } from 'express';
import { PrismaClient } from '@prisma/client';
import { prisma } from '../config/prisma';

/**
 * Creates a mock Express Request object
 */
export function createMockRequest(overrides: Partial<Request> = {}): Partial<Request> {
  return {
    body: {},
    params: {},
    query: {},
    user: { id: 'user-123', email: 'test@test.com', role: 'STUDENT' },
    intern: { id: 'intern-123', name: 'Test Intern', username: 'INT123456' },
    ...overrides,
  };
}

/**
 * Creates a mock Express Response object
 */
export function createMockResponse(): Partial<Response> {
  const res: Partial<Response> = {
    status: vi.fn().mockReturnThis(),
    json: vi.fn().mockReturnThis(),
    send: vi.fn().mockReturnThis(),
  };
  return res;
}

/**
 * Creates a mock NextFunction
 */
export function createMockNext(): NextFunction {
  return vi.fn();
}

/**
 * Creates a mock visit object
 */
export function createMockVisit(overrides: Partial<any> = {}) {
  return {
    id: 'visit-123',
    tokenId: 'VISIT-ABC123',
    studentId: 'user-123',
    propertyId: 'property-123',
    visitDate: new Date('2025-01-15T10:00:00Z'),
    timeSlot: '10:00 AM - 10:20 AM',
    visitOtp: '1234',
    visitOtpVerified: false,
    leadStatus: 'SCHEDULED',
    couponCode: null,
    whatsappNumber: '+919876543210',
    isLocked: false,
    assignedLeadId: null,
    referralCounted: false,
    student: {
      id: 'user-123',
      name: 'Test Student',
      email: 'student@test.com',
      phone: '+919876543210',
    },
    property: {
      id: 'property-123',
      propertyCode: 'PROP-001',
      title: 'Test Property',
      location: 'Test Location',
      price: 10000,
      owner: {
        name: 'Property Owner',
        phone: '+919999999999',
      },
    },
    intern: null,
    ...overrides,
  };
}

/**
 * Creates a mock intern object
 */
export function createMockIntern(overrides: Partial<any> = {}) {
  return {
    id: 'intern-123',
    name: 'Test Intern',
    phone: '+919876543210',
    username: 'INT123456',
    passwordHash: 'hashed-password',
    active: true,
    supervisorId: 'supervisor-123',
    ...overrides,
  };
}

/**
 * Creates a mock property object
 */
export function createMockProperty(overrides: Partial<any> = {}) {
  return {
    id: 'property-123',
    propertyCode: 'PROP-001',
    title: 'Test Property',
    location: 'Test Location, City',
    price: 15000,
    owner: {
      id: 'owner-123',
      name: 'Property Owner',
      phone: '+919999999999',
    },
    ...overrides,
  };
}

/**
 * Creates a mock student/user object
 */
export function createMockStudent(overrides: Partial<any> = {}) {
  return {
    id: 'user-123',
    name: 'Test Student',
    email: 'student@test.com',
    phone: '+919876543210',
    role: 'STUDENT',
    ...overrides,
  };
}

/**
 * Creates a mock supervisor/user object
 */
export function createMockSupervisor(overrides: Partial<any> = {}) {
  return {
    id: 'supervisor-123',
    name: 'Supervisor',
    email: 'supervisor@test.com',
    role: 'SUPERVISOR',
    ...overrides,
  };
}

/**
 * Mock Prisma client for testing
 */
export const mockPrisma = prisma as jest.Mocked<PrismaClient>;

/**
 * Reset all mocks
 */
export function resetMocks() {
  vi.clearAllMocks();
}

/**
 * Setup successful visit creation mocks
 */
export function setupVisitCreationMocks() {
  mockPrisma.visit.findFirst.mockResolvedValue(null);
  mockPrisma.visit.create.mockImplementation((data: any) =>
    Promise.resolve(createMockVisit({ ...data.data, id: 'visit-123' }))
  );
  mockPrisma.coupon.findFirst.mockResolvedValue(null);
  mockPrisma.referral.findFirst.mockResolvedValue(null);
}

/**
 * Setup intern creation mocks
 */
export function setupInternCreationMocks() {
  mockPrisma.intern.create.mockImplementation((data: any) =>
    Promise.resolve(createMockIntern({ ...data.data, id: 'intern-123' }))
  );
}

/**
 * Setup intern assignment mocks
 */
export function setupInternAssignmentMocks() {
  const mockVisit = createMockVisit({
    id: 'visit-123',
    assignedLeadId: null,
    leadStatus: 'SCHEDULED',
  });

  mockPrisma.visit.findUnique.mockResolvedValue(mockVisit);
  mockPrisma.visit.update.mockImplementation((data: any) =>
    Promise.resolve({ ...mockVisit, ...data.data })
  );
  mockPrisma.intern.findUnique.mockResolvedValue(createMockIntern());
}