...
class CouponService {
    async createCoupon(data) {
        ...
        if (!data.code || data.code.trim() === '') {
            throw new app_error_1.AppError('Coupon code is required', 400);
        }
        if (data.value <= 0) {
            throw new app_error_1.AppError('Coupon value must be greater than 0', 400);
        }
        if (data.validFrom >= data.validTo) {
            throw new app_error_1.AppError('validFrom must be before validTo', 400);
        }
        ...
        if (existingCoupon) {
            throw new app_error_1.AppError('Coupon code already exists', 409);
        }
    ...
    async getCouponById(id) {
        ...
        if (!coupon) {
            throw new app_error_1.AppError('Coupon not found', 404);
        }
        return coupon;
    }
    ...
    async validateCoupon(code) {
        ...
        if (!coupon) {
            throw new app_error_1.AppError('Invalid or expired coupon', 400);
        }
        // Check usage limits
        if (coupon.maxUses !== null && coupon.currentUses >= coupon.maxUses) {
            throw new app_error_1.AppError('Coupon usage limit exceeded', 400);
        }
        return coupon;
    }
